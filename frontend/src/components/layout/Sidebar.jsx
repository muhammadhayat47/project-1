          ...
          <NavLink
            ...
            className={({ isActive }) =>
              `flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-colors ${
                isActive
                  ? "bg-gradient-to-r from-glow-500 to-glow-600 text-ink-950 shadow-glow"
...
export default function Sidebar({ mobileOpen = false, onClose }) {
  return (
    <>
      {/* Desktop sidebar */}
      <aside className="hidden w-64 shrink-0 flex-col border-r border-white/10 bg-ink-950/60 text-ink-100 backdrop-blur-xl md:flex">
        <SidebarContent />
      </aside>
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-40 md:hidden">
          <div
            ...
            aria-hidden="true"
          />
          <aside className="relative flex h-full w-72 max-w-[80vw] flex-col border-r border-white/10 bg-ink-950 text-ink-100 shadow-glow-lg animate-rise">
            <button
              onClick={onClose}
